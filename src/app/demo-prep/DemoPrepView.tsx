...
/**
 * Shared view for /demo-prep and the clean channel paths (/demo-prep/call,
 * /demo-prep/test). `source` carries channel attribution when the path
 * provides it; otherwise the ?src= query param or the "site" default applies.
 */
export function DemoPrepView({ source }: { source?: string }) {
  return (
    <div className="min-h-screen bg-anchor">
      ...
      <main className="max-w-3xl mx-auto px-5 py-10 pb-20">
        ...
        <Suspense fallback={null}>
          <ConciergeChat rescheduleUrl={DEMO_URL} source={source} />
        </Suspense>
